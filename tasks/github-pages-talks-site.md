...
6. **Front-matter fields (vendors): `title`, `homepage`, `summary`, `seen_at`** — vendor name, homepage URL, one-line description, event/date string. Parallels the talk field set but tuned to "things to follow up on" rather than session notes. (Field renamed from `url` to `homepage` during Slice 3 implementation, because Jekyll's built-in `page.url` already resolves to the document's permalink — overriding it would break internal page links.)
...
9. **Landing page layout: two labelled sections, rich list per item** — "Talks" (newest-first by `date`, each row showing date + title link + speaker + `summary` blurb) and "Vendors to explore" (alphabetical by `title`, each row showing title link + `homepage` + `summary` + `seen_at`). Index is a single template iterating `site.talks` and `site.vendors` — zero edits when adding new content.
...
**Implementation (Green):** Create `_config.yml` (with `title`, `baseurl: "/ldx3"`, `url: "https://Houms.github.io"`, empty `collections:` block reserved for later slices), `Gemfile` (`source "https://rubygems.org"`, `gem "github-pages", group: :jekyll_plugins`), placeholder `index.md` with a single `<h1>` and a TODO marker, and a `.gitignore` adding `_site/`, `.jekyll-cache/`, and `Gemfile.lock` (Open Question 2 resolved: gitignore the lockfile). Files: `_config.yml` (CREATE), `Gemfile` (CREATE), `index.md` (CREATE), `.gitignore` (CREATE).
...
**Implementation (Green):** Add `collections: vendors: { output: true, permalink: /vendors/:name/ }` and a default layout `vendor` for the collection in `_config.yml`. Create `_layouts/vendor.html` rendering `<h1>{{ page.title }}</h1>`, a metadata line (`<a href="{{ page.homepage }}">{{ page.homepage }}</a> · seen at {{ page.seen_at }}`), `<p>{{ page.summary }}</p>`, then `{{ content }}`. Move `vendor-booth-notes.md` → `_vendors/harness.md`, add front matter (`title: "Harness"`, `homepage: "https://www.harness.io"`, `summary: "<one-liner>"`, `seen_at: "LDX3 2026-06-02"`), strip the body `# H1` and any datestamp blockquote. Files: `_config.yml` (UPDATE), `_layouts/vendor.html` (CREATE), `_vendors/harness.md` (CREATE), `vendor-booth-notes.md` (DELETE). Note: the vendor URL field is named `homepage`, not `url`, because Jekyll reserves `page.url` for the document permalink.
...
**Test (Red):** Build and assert `_site/index.html` contains (a) a "Talks" heading, (b) a "Vendors to explore" heading, (c) all migrated talk titles in the order newest-first by date with title-ascending tie-breaker (Q1 resolution), (d) all vendor titles in alphabetical order, (e) for each talk row: date, title (linked), speaker, summary, (f) for each vendor row: title (linked), homepage, summary, seen_at.
**Implementation (Green):** Rewrite `index.md` with Liquid. Talks sort needs a stable two-key ordering (date DESC, title ASC) — Liquid's `sort` filter is not stable, so use `group_by_exp` to bucket by date, sort the buckets DESC, and sort items within each bucket by title ASC. Vendors use a simple `{% assign vendors = site.vendors | sort: "title" %}`. Then two `<section>` blocks with `<ul>` of rows. Each talk row links via `{{ talk.url | relative_url }}` (Jekyll permalink); each vendor row uses `{{ vendor.url | relative_url }}` for its internal page link (permalink) and `{{ vendor.homepage }}` (front-matter `homepage` field) for the external link. All user-content strings pass through `| escape` before HTML interpolation. Files: `index.md` (UPDATE — replace placeholder from Slice 1).
...
**Test (Red):** Read `README.md` after the change and assert it contains (a) a one-paragraph description of the project, (b) a working link to `https://Houms.github.io/ldx3/`, (c) a "How to add a new talk" section listing the path (`_talks/<slug>.md`) and the four front-matter fields (`title`, `date`, `speaker`, `summary`), (d) a "How to add a new vendor" section listing the path (`_vendors/<slug>.md`) and the four front-matter fields (`title`, `homepage`, `summary`, `seen_at`), (e) a one-line note on running local preview (`bundle install && bundle exec jekyll serve`).
**Implementation (Green):** Rewrite `README.md` with the structure above. Files: `README.md` (UPDATE).
**Refactor:** None.
**Acceptance:** README contains all the elements listed in the test; live-site URL uses the resolved GH username `Houms` (Open Question 3).
